...
func load_tile_scenes() -> void:
	# For now, create simple colored rectangles as placeholder tiles
	# In a full implementation, these would be proper sprite scenes
	wall_tile_scene = create_simple_tile_scene(Color(0.3, 0.3, 0.4))  # Dark gray for walls
	floor_tile_scene = create_simple_tile_scene(Color(0.6, 0.6, 0.7))  # Light gray for floors
	memory_fragment_scene = create_memory_fragment_scene()

func create_simple_tile_scene(color: Color) -> PackedScene:
	var scene = PackedScene.new()
	var node = ColorRect.new()
	node.size = Vector2(tile_size, tile_size)
	node.color = color
	...
	border.add_theme_style_override("normal", create_border_style())
...
func create_memory_fragment_scene() -> PackedScene:
	var scene = PackedScene.new()
	var node = Area2D.new()
	...
	sprite.color = Color(0.8, 0.4, 0.9)  # Purple glow
	...
	# Add to memory_fragments group
	node.add_to_group("memory_fragments")
	
	# Add glow effect (simple animated scale)
	var tween = Tween.new()
...
func render_dungeon() -> void:
	...
	# Render corridors
	render_corridors(dungeon.corridors)
...
func render_room(room: DungeonGenerator.Room) -> void:
		...
		for y in range(room.size.y):
			...
			match tile_type:
				DungeonGenerator.TileType.WALL:
					tile_instance = wall_tile_scene.instantiate()
				DungeonGenerator.TileType.FLOOR, DungeonGenerator.TileType.PLAYER_SPAWN:
					tile_instance = floor_tile_scene.instantiate()
				_:
					tile_instance = floor_tile_scene.instantiate()
...
func spawn_memory_fragment_at(dungeon_pos: Vector2i) -> void:
	var fragment_instance = memory_fragment_scene.instantiate()
	var world_pos = Vector2(
